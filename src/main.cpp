...
.trackList{display:flex;flex-direction:column;gap:6px;margin-top:8px}.trackBtn{width:94%;align-self:center;font-size:17px;padding:9px 12px}.trackOption{display:flex;align-items:center;gap:10px;margin:7px 0}.trackOption input{width:20px;height:20px}.trackOption label{margin:0;font-weight:normal;font-size:16px}
</style></head><body>
<h1 id="title"></h1>
...
<div id="trackSettings"><div class="small">Zaškrtnite vibráciu pre skladby, pri ktorých má motorček vibrovať počas prehrávania.</div><div id="trackFields"></div></div>
...
for(let i=1;i<=8;i++){
 let b=document.createElement('button');b.className='trackBtn';b.textContent='Skladba '+i;b.onclick=()=>cmd('/api/play?track='+i);b.id='t'+i;tracks.appendChild(b);
 let f=document.createElement('div');f.className='field';f.innerHTML='<label>Názov skladby '+i+'</label><input id="tn'+i+'" maxlength="40"><div class="trackOption"><input type="checkbox" id="tv'+i+'"><label for="tv'+i+'">📳 Aktívna vibrácia počas prehrávania</label></div>';trackFields.appendChild(f);
}
