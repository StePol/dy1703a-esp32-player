...
.trackList{display:flex;flex-direction:column;gap:6px;margin-top:8px}.trackBtn{width:94%;align-self:center;font-size:17px;padding:9px 12px}.trackBtn.active{outline:4px solid #555}
</style></head><body>
<h1 id="title"></h1>
...
update();setInterval(update,1000);
    ...
    server.on("/api/play", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasParam("track")) { int track = request->getParam("track")->value().toInt(); if (track >= 1 && track <= 8) { currentTrack = track; player.playTrack(currentTrack); } }
        else player.play();
        lastActivityMs = millis(); request->send(200, "text/plain", "OK");
    });
    server.on("/api/stop", HTTP_GET, [](AsyncWebServerRequest *request) { player.stop(); currentPlaying=false; motor.off(); lastActivityMs=millis(); request->send(200,"text/plain","OK"); });
    server.on("/api/pause", HTTP_GET, [](AsyncWebServerRequest *request) { player.pause(); currentPlaying=false; motor.off(); lastActivityMs=millis(); request->send(200,"text/plain","OK"); });
...
void handlePendingWiFiRestart() {
    ...
    Serial.println(F("Používateľ uložil nastavenia -> reštartujem ESP32"));
    Serial.flush();
    delay(50);
    ESP.restart();
}
...
void handleWiFi() {
    if (!wifiActive) return;
    if (wifiIsSta) {
        if (WiFi.status() != WL_CONNECTED) { Serial.println(F("WiFi STA bolo odpojené -> skúšam znovu AP")); stopWiFi(); startAP(); return; }
        lastActivityMs = millis(); return;
    }
    unsigned long now=millis(); uint8_t clients=WiFi.softAPgetStationNum();
...
bool handleButtons() {
    ...
    for(auto &b:buttons){bool state=digitalRead(b.pin);if(state!=b.lastState&&(now-b.lastChangeMs)>BTN_DEBOUNCE_MS){b.lastChangeMs=now;b.lastState=state;if(state==LOW){player.playTrack(b.trackNumber);currentTrack=b.trackNumber;Serial.printf("Tlačidlo %u -> skladba %u\n",b.pin,b.trackNumber);startWiFi();pressed=true;}}}
    return pressed;
}

void maybeEnterLightSleep() {
    if(wifiActive)return;
    unsigned long now=millis();if(now-lastActivityMs<INACTIVITY_TIMEOUT_MS)return;if(currentPlaying)return;
    motor.off();Serial.println(F("Nečinnosť -> light sleep (WiFi OFF)"));Serial.flush();
    esp_sleep_enable_gpio_wakeup();for(uint8_t pin:TRACK_BUTTON_PINS)gpio_wakeup_enable((gpio_num_t)pin,GPIO_INTR_LOW_LEVEL);esp_sleep_enable_timer_wakeup(LIGHT_SLEEP_CHECK_INTERVAL_MS*1000ULL);
    esp_light_sleep_start();for(uint8_t pin:TRACK_BUTTON_PINS)gpio_wakeup_disable((gpio_num_t)pin);
    if(esp_sleep_get_wakeup_cause()==ESP_SLEEP_WAKEUP_GPIO){Serial.println(F("Prebudenie tlačidlom"));lastActivityMs=millis();}else Serial.println(F("Periodické prebudenie (kontrola stavu)"));
}

void setup() {
    Serial.begin(115200);delay(300);setupButtons();loadSettings();player.begin(DY_BAUD_RATE,DY_RX_PIN,DY_TX_PIN);player.setVolume(currentVolume);battery.begin();motor.begin();lastActivityMs=millis();Serial.println(F("DY1703A ESP32 Player – ready"));startWiFi();
}

void loop() {
    if(handleButtons())lastActivityMs=millis();player.poll();
    static unsigned long lastBatteryRead=0,lastMotorPoll=0;unsigned long now=millis();
    if(now-lastMotorPoll>=MOTOR_POLL_INTERVAL_MS){lastMotorPoll=now;currentPlaying=player.isPlaying();motor.set(currentPlaying);if(currentPlaying)lastActivityMs=now;}
    if(now-lastBatteryRead>=BATTERY_READ_INTERVAL_MS){lastBatteryRead=now;batteryMv=battery.readVoltageMv();batteryPct=battery.readPercent();Serial.printf("Batéria: %lu mV (~%u%%)\n",batteryMv,batteryPct);}
    handlePendingWiFiRestart();handleWiFi();maybeEnterLightSleep();
}
